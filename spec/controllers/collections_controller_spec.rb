require 'spec_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

describe CollectionsController do

  #include Devise::TestHelpers

  # This should return the minimal set of attributes required to create a valid
  # Collection. As you add validations to Collection, be sure to
  # update the return value of this method accordingly.
  def valid_attributes
    {:name => 'Sample collection',
      :user => FactoryGirl.create(:user)
    }
  end

  fixtures :collections # include common collection

  describe "GET index" do
    
    it 'doesnt show others collections nor common collection (NOT LOGGED USER)' do
      c1 = FactoryGirl.build(:collection, :user_id => 1)
      c1.save(:validation => false)   

      get :index
      assigns(:records).should == [] 
    end

    it "shows only user's collection (LOGGED USER)" do
      user1 = Factory.create(:user)
      user2 = Factory.create(:user)
      sign_in user1

      c1 = FactoryGirl.create(:collection, :user => user1)
      c2 = FactoryGirl.create(:collection, :user => user2)

      get :index
      assigns(:records).should == [c1] 
    end



  end

#  describe "GET show" do
#    it "assigns the requested collection as @record" do
#      record = Collection.create! valid_attributes
#      get :show, :id => record.id.to_s
#      assigns(:record).should eq(record)
#    end
#  end
#
#  describe "GET new" do
#    it "assigns a new collection as @record" do
#      get :new
#      assigns(:record).should be_a_new(Collection)
#    end
#  end
#
#  describe "GET edit" do
#    it "assigns the requested collection as @record" do
#      record = Collection.create! valid_attributes
#      get :edit, :id => record.id.to_s
#      assigns(:record).should eq(record)
#    end
#  end
#
#  describe "POST create" do
#    describe "with valid params" do
#      it "creates a new Collection" do
#        expect {
#          post :create, :record => valid_attributes
#        }.to change(Collection, :count).by(1)
#      end
#
#      it "assigns a newly created collection as @record" do
#        post :create, :record => valid_attributes
#        assigns(:record).should be_a(Collection)
#        assigns(:record).should be_persisted
#      end
#
#      it "redirects to the created collection" do
#        post :create, :record => valid_attributes
#        #response.should redirect_to(Collection.last)
#        response.should redirect_to(collections_url)
#      end
#    end
#
#    describe "with invalid params" do
#      it "assigns a newly created but unsaved collection as @record" do
#        # Trigger the behavior that occurs when invalid params are submitted
#        Collection.any_instance.stub(:save).and_return(false)
#        post :create, :record => {}
#        assigns(:record).should be_a_new(Collection)
#      end
#
#      it "re-renders the 'new' template" do
#        # Trigger the behavior that occurs when invalid params are submitted
#        Collection.any_instance.stub(:save).and_return(false)
#        post :create, :record => {}
#        #response.should render_template("new")
#        # can't figure out what it should render
#        #response.should render_template(:partial => 'messages')
#      end
#    end
#  end
#
#  describe "PUT update" do
#    describe "with valid params" do
#      it "updates the requested collection" do
#        record = Collection.create! valid_attributes
#        # Assuming there are no other collections in the database, this
#        # specifies that the Collection created on the previous line
#        # receives the :update_attributes message with whatever params are
#        # submitted in the request.
#        #Collection.any_instance.should_receive(:update_attributes).with({'these' => 'params'})
#        Collection.any_instance.should_receive(:save!)
#        put :update, :id => record.id, :record => {'name' => 'Changed name'}
#      end
#
#      it "assigns the requested collection as @record" do
#        record = Collection.create! valid_attributes
#        put :update, :id => record.id, :record => valid_attributes
#        assigns(:record).should eq(record)
#      end
#
#      it "redirects to the collection" do
#        record = Collection.create! valid_attributes
#        put :update, :id => record.id, :record => valid_attributes
#        #response.should redirect_to(record)
#        response.should redirect_to(collections_url)
#      end
#    end
#
#    describe "with invalid params" do
#      it "assigns the collection as @record" do
#        record = Collection.create! valid_attributes
#        # Trigger the behavior that occurs when invalid params are submitted
#        Collection.any_instance.stub(:save).and_return(false)
#        put :update, :id => record.id.to_s, :record => {}
#        assigns(:record).should eq(record)
#      end
#
#      it "re-renders the 'edit' template" do
#        record = Collection.create! valid_attributes
#        # Trigger the behavior that occurs when invalid params are submitted
#        Collection.any_instance.stub(:save).and_return(false)
#        put :update, :id => record.id.to_s, :record => {}
#        #response.should render_template("edit")
#        # can't figure out what it should render
#        #response.should render_template(:partial => 'on_update')
#      end
#    end
#  end
#
#  describe "DELETE destroy" do
#    it "destroys the requested collection" do
#      record = Collection.create! valid_attributes
#      expect {
#        delete :destroy, :id => record.id.to_s
#      }.to change(Collection, :count).by(-1)
#    end
#
#    it "redirects to the collections list" do
#      record = Collection.create! valid_attributes
#      delete :destroy, :id => record.id.to_s
#      response.should redirect_to(collections_url)
#    end
#  end

end
